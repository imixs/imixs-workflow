...
/**
 * The FileUploadController is a conversation scoped bean and used to hold the upladed files and
 * transfere the to the accoicated workitem. The AjaxFileUploadServlet injects this bean to provide
 * new file data.
 * 
...
public class FileUploadController implements Serializable {
  ...
  /**
   * This method set the current workitem and starts a new conversation. With this mechanism the
   * fileUploadController bean can be used in multiple browser tabs or browser sessions.
   * 
   * @param workitem
   */
  public void setWorkitem(ItemCollection workitem) {
    ...
    if (workitem != null) {
      ...
      if (conversation.isTransient()) {
        conversation.setTimeout(((HttpServletRequest) FacesContext.getCurrentInstance()
            .getExternalContext().getRequest()).getSession().getMaxInactiveInterval() * 1000);
        conversation.begin();
  ...
  /**
   * Returns the current conversation id. This id is passed to the AjaxFileUploadServelt URIs so
   * make sure that the correct FileUploadController is injected.
   * 
   * @return
   */
  ...
  /**
   * This method is called by the AjaxFileUpload Servlet. The method adds the file to the workitem
   * but also updates the list of temporary files, which are not yet persisted.
   * 
   * @param document
   * @param aFilename
   */
  ...
  /**
   * Removes a file object from a given workitem. Here we operate on a given workitem as the
   * imixsFileUpload.xhtml has no idea of he current conversation scoped controller.
   * 
   * @param sFilename - filename to be removed
   * @return - null
   */
