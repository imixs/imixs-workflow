...
public class ErrorHandler {
  ...
  /**
   * The Method expects a PluginException and adds the corresponding Faces Error Message into the
   * FacesContext.
   * 
   * If the PluginException was thrown from the RulePLugin then the method test this exception for
   * ErrorParams and generate separate Faces Error Messages for each param.
   */
  public static void handlePluginException(PluginException pe) {
    ...
    if (RulePlugin.class.getName().equals(pe.getErrorContext())
        ...
        && pe.getErrorParameters() != null && pe.getErrorParameters().length > 0) {
    ...
    logger.warning("ErrorHandler cauth PluginException - error code=" + pe.getErrorCode() + " - "
        + pe.getMessage());
  ...
  /**
   * The Method expects a ModelException and adds the corresponding Faces Error Message into the
   * FacesContext.
   * 
   * In case of a model exception, the exception message will become part of the error message.
   * ErrorParams are not supported by a ModelException.
   */
  public static void handleModelException(ModelException me) {
    ...
    logger.warning("ErrorHandler cauth ModelException - error code=" + me.getErrorCode() + " - "
        + me.getMessage());
    if (logger.isLoggable(Level.FINE)) {
  ...
  /**
   * This helper method adds a error message to the faces context, based on the data in a
   * WorkflowException. This kind of error message can be displayed in a page using:
   * 
   * <code>
