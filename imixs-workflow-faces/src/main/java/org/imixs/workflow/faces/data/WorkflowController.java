...
/**
 * 
 * The WorkflowController is a @ConversationScoped CDI bean to control the processing life cycle of
 * a workitem in JSF an application. The bean can be used in single page applications, as well for
 * complex page flows. The controller supports bookmarkable URLs.
 * <p>
 * The WorkflowController fires CDI events from the type WorkflowEvent. A CDI bean can observe these
 * events to participate in the processing life cycle.
 * <p>
 * To load a workitem the methods load(id) and onLoad() can be used. The method load expects a valid
 * uniqueId of a workItem to be loaded. The onLoad() method extracts the uniqueid from the query
 * parameter 'id'. This is the recommended way to support bookmarkable URLs when opening a JSF page
 * with the data of a workitem. The onLoad method can be triggered by an jsf viewAction placed in
 ...
 * <p>
 * After a workitem was processed, the conversation is automatically closed. Stale conversations
 * will automatically timeout with the default session timeout.
 * <p>
 * After each call of the method process the Post-Redirect-Get is initialized with the default URL
 * from the start of the conversation. If an alternative action result is provided by the workflow
 * engine, the WorkflowController automatically redirects the user to the new form outcome. This
 * guarantees bookmarkable URLs.
 * <p>
...
public class WorkflowController extends AbstractDataController implements Serializable {
   ...
   * <p>
   * The Method also catches PluginExceptions and adds the corresponding Faces Error Message into
   * the FacesContext. In case of an exception the WorkflowEvent WORKITEM_AFTER_PROCESS will not be
   * fired.
   * <p>
   * In case the processing was successful, the current conversation will be closed. In Case of an
   * Exception (e.g PluginException) the conversation will not be closed, so that the current
   * workitem data is still available.
   * 
   * @return the action result provided in the 'action' property or evaluated from the default
   *         property 'txtworkflowResultmessage' from the ActivityEntity
  ...
  public String process() throws PluginException, ModelException {
    ...
    try {
      ...
      logger.finest("......fire WORKITEM_BEFORE_PROCESS event: ' in "
      ...
      if (logger.isLoggable(Level.FINEST)) {
        logger
            .finest("......[process] '" + getWorkitem().getItemValueString(WorkflowKernel.UNIQUEID)
    ...
    if (actionResult != null && actionResult.startsWith("http")) {
      ...
      try {
        ...
        logger.warning(
            "Failed to redirect action result: " + externalURL + " - Error: " + e.getMessage());
        e.printStackTrace();
      }
    }
  ...
  /**
   * This method processes the current workItem with the provided eventID. The method can be used as
   * an action or actionListener.
   * 
  ...
  /**
   * This method returns a List of workflow events assigned to the corresponding '$taskid' and
   * '$modelversion' of the current WorkItem.
   * 
   * @return
   */
