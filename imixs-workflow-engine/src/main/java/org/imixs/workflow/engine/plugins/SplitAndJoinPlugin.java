...
/**
 * The Imixs Split&Join Plugin provides functionality to create and update sub-process instances
 * from a workflow event in an origin process. It is also possible to update the origin process from
 * the sub-process instance.
 * 
 * The plugin evaluates the txtactivityResult and the items with the following names:
 * 
 ...
 * 
 * 
 * A subprocess will contain the $UniqueID of the origin process stored in the property
 * $uniqueidRef. The origin process will contain a link to the subprocess stored in the property
 * txtworkitemRef. So both workitems are linked together.
 * 
 * 
 * @author Ralph Soika
 * @version 1.0
...
public class SplitAndJoinPlugin extends AbstractPlugin {
   ...
   * 
   * @throws @throws ProcessingErrorException @throws AccessDeniedException @throws
   * 
   */
  ...
  public ItemCollection run(ItemCollection adocumentContext, ItemCollection adocumentActivity)
    ...
    ItemCollection evalItemCollection =
        getWorkflowService().evalWorkflowResult(adocumentActivity, adocumentContext, false);
  ...
  /**
   * This method expects a list of Subprocess definitions and create for each definition a new
   * subprocess. The reference of the created subprocess will be stored in the property
   * txtworkitemRef of the origin workitem
   * 
   * 
   ...
   * 
   *
   * Both workitems are connected to each other. The subprocess will contain the $UniqueID of the
   * origin process stored in the property $uniqueidRef. The origin process will contain a link to
   * the subprocess stored in the property txtworkitemRef.
   *
   * The tag 'action' is optional and allows to overwrite the action result evaluated by the
   * ResultPlugin.
   * 
   ...
   */
  protected void createSubprocesses(final List<String> subProcessDefinitions,
      final ItemCollection originWorkitem)
    ...
    for (String processValue : subProcessDefinitions) {
      ...
      if (processData != null) {
        ...
        workitemSubProcess.replaceItemValue(WorkflowService.UNIQUEIDREF,
            originWorkitem.getUniqueID());
  ...
  /**
   * This method expects a list of Subprocess definitions and updates each matching existing
   * subprocess.
   * 
   ...
   * </code>
   * 
   * Subprocesses and the origin process are connected to each other. The subprocess will contain
   * the $UniqueID of the origin process stored in the property $uniqueidRef. The origin process
   * will contain a link to the subprocess stored in the property txtworkitemRef.
   * 
   * @param subProcessDefinitions
   * @param originWorkitem
   * @throws AccessDeniedException
   ...
   * @throws ModelException
   */
  protected void updateSubprocesses(final List<String> subProcessDefinitions,
      final ItemCollection originWorkitem)
      throws AccessDeniedException, ProcessingErrorException, PluginException, ModelException {
      ...
      if (processData != null) {
        ...
        for (String subProcessRef : subProcessRefList) {
          ...
          if (Pattern.compile(model_pattern).matcher(subModelVersion).find()
            ...
            // now clone the field list...
            copyItemList(processData.getItemValueString("items"), originWorkitem,
                workitemSubProcess);
   ...
   * The origin workitem will be loaded by the $uniqueidRef stored in the subprocess
   * 
   * The processing definition for the origin process is expected in the following format
   * 
   * <code>
   * 	  <event>20</event>
   *    <items>namTeam,_sub_data</items>
   ...
   */
  @SuppressWarnings("unchecked")
  protected void updateOrigin(final String originProcessDefinition,
      final ItemCollection subprocessWorkitem)
      throws AccessDeniedException, ProcessingErrorException, PluginException, ModelException {
  ...
  /**
   * This Method copies the fields defined in 'items' into the targetWorkitem. Multiple values are
   * separated with comma ','.
   * <p>
   * In case a item name contains '|' the target field name will become the right part of the item
   * name.
   * <p>
   * Example: {@code
   ...
   * 
   * <p>
   * Optional also reg expressions are supported. In this case mapping of the item name is not
   * supported.
   * <p>
   * Example: {@code
