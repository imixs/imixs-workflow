...
public class WorkflowScheduler implements Scheduler {
  ...
  /**
   * This method checks if a workitem (doc) is in due. There are 4 different cases which will be
   * compared: The case is determined by the keyScheduledBaseObject of the activity entity
   * 
   * Basis : keyScheduledBaseObject "last process"=1, "last Modification"=2 "Creation"=3 "Field"=4
   * 
   * The logic is not the best one but it works. So we are open for any kind of improvements
   * 
    ...
    try {
      ...
      try {
        ...
        if (iOffsetUnit < 1 || iOffsetUnit > 4) {
          logger.warning("error parsing delay in ActivityEntity "
      ...
      switch (iCompareType) {
        ...
        case 4: {
          ...
          if (!doc.hasItem(sNameOfField)) {
            logger.finest(
                "......" + suniqueid + ": CompareType =" + sNameOfField + " no value found!");
            return false;
          }
  ...
  /**
   * This method adds workdays (MONDAY - FRIDAY) to a given calendar object. If the number of days
   * is negative than this method subtracts the working days from the calendar object.
   * 
   * 
   * @param cal
   * @param days
  ...
  public Calendar addWorkDays(final Calendar baseDate, final int days) {
    ...
    if (resultDate != null) {
      logger.finest("......addWorkDays (" + baseDate.getTime() + ") + " + days + " = ("
          + resultDate.getTime() + ")");
    }
    return resultDate;
  }
  ...
  /**
   * This method process scheduled workitems. The method updates the property 'datLastRun'
   * 
   * Because of bug: https://java.net/jira/browse/GLASSFISH-20673 we check the imixsDayOfWeek
   * 
   * @param timer
  ...
  public ItemCollection run(ItemCollection configItemCollection) throws SchedulerException {
    ...
    schedulerService.logMessage("..." + iProcessWorkItems + " workitems processed",
        configItemCollection, null);
  ...
  /**
   * This method collects all scheduled workflow events. A scheduled workflow event is identified by
   * the attribute keyScheduledActivity="1"
   * 
  ...
  protected Collection<ItemCollection> findScheduledEvents(String aModelVersion) throws Exception {
    ...
    for (ItemCollection aprocessentity : colProcessList) {
      ...
      Collection<ItemCollection> aActivityList =
          modelService.getModel(aModelVersion).findAllEventsByTask(processid);

      for (ItemCollection aactivityEntity : aActivityList) {
        logger.finest(
            "......analyse acitity '" + aactivityEntity.getItemValueString("txtname") + "'");
   ...
   * A scheduled event element can define a selector (txtscheduledview). If no selector is defined,
   ...
   * }
   * <p>
   * In case an old modelversion was deleted, the method tries to migrate to the lates model
   * version. (issue #482)
   * 
