...
/**
 * The DocumentService is used to save and load instances of ItemCollections into a Database. The
 * DocumentService throws an AccessDeniedException if the CallerPrincipal is not allowed to save or
 * read a specific Document from the database. So the DocumentService can be used to save business
 * objects into a database with individual read- or writeAccess restrictions.
 * <p>
 * The Bean holds an instance of an EntityPersistenceManager for the persistence unit
 * 'org.imixs.workflow.jpa' to manage the Document entity bean class. The Document entity bean is
 * used to store the attributes of a ItemCollection into the connected database.
 * <p>
 * The save() method persists any instance of an ItemCollection. If a ItemCollection is saved the
 * first time the DocumentService generates the attribute $uniqueid which will be included in the
 * ItemCollection returned by this method. If a ItemCollection was saved before the method updates
 * the corresponding Document Object.
 * <p>
 * The load() and find() methods are used to read ItemCollections from the database. The remove()
 * method deletes a saved ItemCollection from the database.
 * <p>
 * All methods expect and return Instances of the object org.imixs.workflow.ItemCollection which is
 * no entity EJB. So these objects are not managed by any instance of an EntityPersistenceManager.
 * <p>
 ...
 * org.imixs.workflow.ItemCollection the method also manages the read- and writeAccess for each
 * instance of an ItemCollection. Therefore the save() method scans an ItemCollection for the
 * attributes '$ReadAccess' and '$WriteAccess'. The DocumentService verifies in each call of the
 * save() load(), remove() and find() methods if the current callerPrincipal is granted to the
 * affected entities. If an ItemCollection was saved with read- or writeAccess the access to an
 * Instance of a saved ItemCollection will be protected for a callerPrincipal with missing read- or
 * writeAccess.
 * <p>
 * 
...
public class DocumentService {
   ...
   * <p>
   * A client can extend the list of user groups associated with a userId by reacting on the CDI
   * event 'UserGrouptEvent'.
   * 
   * @see UserGroupEvent
   * @return
   */
  public List<String> getUserNameList() {
    ...
    String roleList =
        "org.imixs.ACCESSLEVEL.READERACCESS,org.imixs.ACCESSLEVEL.AUTHORACCESS,org.imixs.ACCESSLEVEL.EDITORACCESS,org.imixs.ACCESSLEVEL.MANAGERACCESS,"
            + accessRoles;
  ...
  /**
   * This method returns true, if at least one element of the current UserNameList is contained in a
   * given name list. The comparison is case sensitive!
   * 
   * @param nameList
   * @return
   */
  ...
  /**
   * This Method saves an ItemCollection into the database. If the ItemCollection is saved the first
   * time the method generates a uniqueID ('$uniqueid') which can be used to identify the
   * ItemCollection by its ID. If the ItemCollection was saved before, the method updates the
   * existing ItemCollection stored in the database.
   * 
   * <p>
   * The Method returns an updated instance of the ItemCollection containing the attributes
   * $modified, $created, and $uniqueId
   * 
   * <p>
   * The method throws an AccessDeniedException if the CallerPrincipal is not allowed to save or
   * update the ItemCollection in the database. The CallerPrincipial should have at least the access
   * Role org.imixs.ACCESSLEVEL.AUTHORACCESS
   * 
   * <p>
   * The method adds/updates the document into the lucene index.
   * 
   * <p>
   * The method returns a itemCollection without the $VersionNumber from the persisted entity. (see
   * issue #226)
   * <p>
   * 
   * <p>
   * issue #230:
   * 
   * The document will be marked as 'saved' so that the methods load() and getDocumentsByQuery() can
   * evaluate this flag. Depending on the state, the methods can decide the correct behavior. In
   * general we detach a document in the load() and getDocumentsByQuery() method. This is for
   * performance reasons and the fact, that a ItemCollection can hold byte arrays which will be
   * copied by reference. In cases where these methods are called after a document was saved
   * (document is now managed), in one single transaction, the detach call will discard the changes
   * made by the save() method. For that reason we flag the entity and evaluate this flag in the
  ...
  public ItemCollection save(ItemCollection document) throws AccessDeniedException {
    ...
    if (persistedDocument == null) {
      ...
      if (ItemCollection.createByReference(persistedDocument.getData())
          .getItemValueBoolean(IMMUTABLE)) {
        throw new AccessDeniedException(OPERATION_NOTALLOWED,
            "Operation not allowed, document is immutable!");
      }
    ...
    if (debug) {
      logger.finest("......save - ID=" + document.getUniqueID() + " managed version="
          + persistedDocument.getVersion());
    }
    ...
    if ((!persistedDocument.getId().equals(document.getUniqueID())) || (!persistedDocument
        .getCreated().getTime().equals(document.getItemValueDate("$created")))) {
    ...
    if (!disableOptimisticLocking && document.hasItem(VERSION)
        && document.getItemValueInteger(VERSION) > 0) {
     ...
     * issue #226
     * 
     * No em.flush(), em.detach() or em.clear() is needed here. Finally we remove the $version
     * property from the ItemCollection returned to the client. This is important for the case that
     * the method is called multiple times in one single transaction.
     */
    ...
    if (debug) {
      logger.fine("...'" + document.getUniqueID() + "' saved in "
          + (System.currentTimeMillis() - lSaveTime) + "ms");
    }
  ...
  /**
   * This method adds a single document into the to the Lucene index. Before the document is added
   * to the index, a new eventLog is created. The document will be indexed after the method
   * flushEventLog is called. This method is called by the LuceneSearchService finder methods.
   * <p>
   * The method supports committed read. This means that a running transaction will not read an
   * uncommitted document from the Lucene index.
   * 
   * 
  ...
  /**
   * This method adds a new eventLog for a document to be deleted from the index. The document will
   * be removed from the index after the method fluschEventLog is called. This method is called by
   * the LuceneSearchService finder method only.
   * 
   * 
  ...
  /**
   * This method saves a workitem in a new transaction. The method can be used by plugins to isolate
   * a save request from the current transaction context.
   * 
  ...
  /**
   * This method loads an ItemCollection from the Database. The method expects a valid $uniqueID to
   * identify the Document entity saved before into the database. The method returns null if no
   * Document with the corresponding ID exists.
   * <p>
   * The method checks if the CallerPrincipal has read access to Document stored in the database. If
   * not, the method returns null.
   * <p>
   * <strong>Note:</strong> The method dose not throw an AccessDeniedException if the user is not
   * allowed to read the entity to prevent a aggressor with informations about the existence of that
   * specific Document.
   * <p>
   ...
   * issue #230
   * 
   * In case a document is not flagged (not saved during same transaction), we detach the loaded
   * entity. In case a document is flagged (saved during save transaction) we may not detach it, but
   * make a deepCopy (clone) of the document instance. This will avoid the effect, that data written
   * to a document get lost in a long running transaction with save and load calls.
   * 
   * @param id - the $uniqueid of the ItemCollection to be loaded
   * @return ItemCollection object or null if the Document dose not exist or the CallerPrincipal hat
  ...
  public ItemCollection load(String id) {
    ...
    if (persistedDocument != null && isCallerReader(persistedDocument)) {
      ...
      if (persistedDocument.isPending()) {
        ...
        if (debug) {
          logger.finest("......clone manged entity '" + id + "' pending status="
              + persistedDocument.isPending());
        }
      ...
      if (debug) {
        logger.fine("...'" + result.getUniqueID() + "' loaded in "
            + (System.currentTimeMillis() - lLoadTime) + "ms");
      }
      return result;
    } else
      return null;
  ...
  /**
   * This method removes an ItemCollection from the database. If the CallerPrincipal is not allowed
   * to access the ItemColleciton the method throws an AccessDeniedException.
   * <p>
  ...
  /**
   * Returns the total hits for a given search query. The provided search term will be extended with
   * a users roles to test the read access level of each workitem matching the search term. The
  ...
  /**
   * The method returns a list of ItemCollections from the search-index. The method expects an valid
   * Lucene search term.
   * <p>
   * The method returns only ItemCollections which are readable by the CallerPrincipal. With the
  ...
  /**
   * The method returns a sorted list of ItemCollections from the search-index. The result list can
   * be sorted by a sortField and a sort direction.
   * <p>
   * The method expects an valid Lucene search term. The method returns only ItemCollections which
   * are readable by the CallerPrincipal. With the pageSize and pageNumber it is possible to
   * paginate.
   * 
   * @param searchTerm  - Lucene search term
   * @param pageSize    - total docs per page
  ...
  public List<ItemCollection> find(String searchTerm, int pageSize, int pageIndex, String sortBy,
    ...
    boolean debug = logger.isLoggable(Level.FINE);
    if (debug) {
      logger.finest("......find - SearchTerm=" + searchTerm + "  , pageSize=" + pageSize
          + " pageNumber=" + pageIndex + " , sortBy=" + sortBy + " reverse=" + sortReverse);
    }
    // create sort object
    ...
    return indexSearchService.search(searchTerm, pageSize, pageIndex, sortOrder, defaultOperator,
        false);

  }

  /**
   * The method returns a sorted list of Document Stubs from the search-index. A document stub
   * contains only the items stored in the search index. These items can be defined by the property
   * <code>lucence.indexFieldListStore</code>. See the LuceneUpdateService for details.
   * <p>
   ...
   * @see org.imixs.workflow.engine.index.SearchService
   */
  public List<ItemCollection> findStubs(String searchTerm, int pageSize, int pageIndex,
      String sortBy, boolean sortReverse) throws QueryException {
    boolean debug = logger.isLoggable(Level.FINE);
    if (debug) {
      logger.finest("......find - SearchTerm=" + searchTerm + "  , pageSize=" + pageSize
          + " pageNumber=" + pageIndex + " , sortBy=" + sortBy + " reverse=" + sortReverse);
  ...
  /**
   * Returns an unordered list of all documents of a specific type. The method throws an
   * InvalidAccessException in case no type attribute is defined.
   * 
   * @param type
   * @return
    ...
    for (Document doc : documentList) {
      if (isCallerReader(doc)) {
        ...
        if (doc.isPending()) {
          // we clone but do not detach
          if (debug) {
            logger.finest("......clone manged entity '" + doc.getId() + "' pending status="
                + doc.isPending());
          }
  ...
  /**
   * This method creates a backup of the result set form a Lucene search query. The document list
   * will be stored into the file system. The method stores the Map from the ItemCollection to be
   * independent from version upgrades. To manage large dataSets the method reads the documents in
   * smaller blocks
   * 
   * @param entities
   * @throws IOException
  ...
  public void restore(String filePath) throws IOException {
    ...
    while (true) {
      try {
        ...
        if (icount >= JUNK_SIZE) {
          icount = 0;
          logger.info("...restored " + totalcount + " document in "
              + (System.currentTimeMillis() - l) + "ms....");
        ...
        errorCount++;
        logger.warning("...error importing workitem at position " + (totalcount + errorCount)
            + " Error: " + e.getMessage());
  ...
  public boolean isAuthor(ItemCollection itemcol) {
    ...
    /**
     * 2.) org.imixs.ACCESSLEVEL.MANAGERACCESS or org.imixs.ACCESSLEVEL.EDITOR Always true - grant
     * writeaccess.
     */
    if (ctx.isCallerInRole(ACCESSLEVEL_MANAGERACCESS)
        || ctx.isCallerInRole(ACCESSLEVEL_EDITORACCESS))
  ...
  /**
   * This method udates the metadata of a new loaded ItemCollection based on the document
   * attributes.
   * <p>
   * The metadata which is updated is:
   * <ul>
   * <li>$version - set to current version if OptimisticLocking is not disabled</li>
  ...
  private boolean isCallerReader(Document document) {
    ...
    /**
     * 2.) org.imixs.ACCESSLEVEL.EDITOR org.imixs.ACCESSLEVEL.AUTHOR ACCESSLEVEL.READER
     * 
     * check read access
     */
