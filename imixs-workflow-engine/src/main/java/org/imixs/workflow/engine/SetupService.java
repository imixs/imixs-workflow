 ...
 * 'MODEL_DEFAULT_DATA'. This variable can point to multiple model resources separated by a ';'. A
 * model resource file must have the file extension '.bpmn'.
 * <p>
 * The variable can be defined also in the imixs.properties file. In this case the variable is
 * named: 'model.default.data'.
 * <p>
 ...
 * <p>
 * With the method 'getModelCount' the service returns the current status of the workflow engine by
 * returning the count of valid workflow models.
 * <p>
 * The SetupSerivce has a migration method to migrate old Workflow Schedulers into the new Scheduler
 * concept. The method migrateWorkflowScheduler is nust for migration and can be deprecated in
 * future releases.
 * 
 * @author rsoika
 * @version 1.0
...
public class SetupService {
      ...
      if (modelResource.endsWith(".bpmn") || modelResource.endsWith(".xml")) {
        ...
        try {
          ...
          // issue #600 return; // MODEL_INITIALIZED;
            ...
            | SAXException e) {
          throw new RuntimeException("Failed to load model configuration: " + e.getMessage()
              + " check 'model.default.data'", e);
        } finally {
          if (inputStream != null) {
            try {
              inputStream.close();
  ...
  /**
   * this method imports an xml entity data stream. This is used to provide model uploads during the
   * system setup. The method can also import general entity data like configuration data.
   * 
   * @param event
   * @throws Exception
   */
  public void importXmlEntityData(byte[] filestream) {
    ...
    try {
      ...
      if (ecol.getDocument().length > 0) {
        ...
        logger.fine(
            "importXmlEntityData - " + ecol.getDocument().length + " entries sucessfull imported");
      }
  ...
  /**
   * This method migrates the deprecated WorkflowScheduelr configuraiton into the new Imixs
   * Scheduler API
   */
