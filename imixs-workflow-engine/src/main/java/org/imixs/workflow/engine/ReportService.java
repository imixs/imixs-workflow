 ...
 * 
 * A Report Entity is identified by its name represented by the attribute 'name' So a ReportService
 * Implementation should ensure that name is a unique key for the report entity.
 * 
 * Also each report entity holds a EQL Query in the attribute "txtquery". this eql statement will be
 * processed by the processQuery method and should return a collection of entities defined by the
 * query.
 * 
 * 
 * @author Ralph Soika
...
public class ReportService {
  ...
  /**
   * Returns a Report Entity by its identifier. The identifier can either be the $uniqueId of the
   * report or the report name. The method returns null if no report with the given identifier
   * exists.
   * 
  ...
  /**
   * updates a Entity Report Object. The Entity representing a report must have at least the
   * attributes : txtQuery, numMaxCount, numStartPost, txtName.
   * 
   * txtName is the unique key to be use to get a query.
   * 
   * The method checks if a report with the same key allready exists. If so this report will be
   * updated. If no report exists the new report will be created
   * 
   * @param report
   ...
   * <p>
   * The method executes the lucene search query defined by the Report. The values of the returned
   * entities will be cloned and formated in case a itemList is provided.
   * <p>
   * The method parses the attribute txtname for a formating expression to format the item value.
   * E.g.:
   * <p>
   * {@code
   * 
   ...
   * </pre>
   * <p>
   * In this example the literal ?{date_from} will be replaced with the given value provided in the
   * param map.
   * <p>
   * 
   ...
   * @param params     - optional parameter list to be mapped to the JQPL statement
  ...
  public List<ItemCollection> getDataSource(ItemCollection reportEntity, int pageSize,
      int pageIndex, String sortBy, boolean sortReverse, Map<String, String> params)
      throws QueryException {
      ...
      while (iter.hasNext()) {
        ...
        if (query.indexOf("{" + sKeyName + "}") > -1) {
          ...
          if (query.indexOf("?" + sKeyName) > -1) {
            query = query.replace("?" + sKeyName, sParamValue);
            logger.warning(
                "......query definition in Report '" + reportEntity.getItemValueString("txtname")
    ...
    List<ItemCollection> result =
        documentService.find(query, pageSize, pageIndex, sortBy, sortReverse);
   ...
   * @throws IOException
   */
  public FileData transformDataSource(ItemCollection report, List<ItemCollection> data,
      String fileName) throws JAXBException, TransformerException, IOException {
  ...
  /**
   * This method parses a <date /> xml tag and computes a dynamic date by parsing the attributes:
   * 
   * DAY_OF_MONTH
   * 
   * DAY_OF_YEAR
  ...
  /**
   * This method replaces all occurrences of {@code<date>} tags with the corresponding dynamic date.
   * See computeDynamicdate.
   * 
   * @param content
   * @return
   */
  ...
  /**
   * This methode updates the a itemCollection with the attributes supported by another
   * itemCollection without the $uniqueid
   * 
   * @param aworkitem
   * 
   */
  ...
  /**
   * This method returns true if the attribute name can be updated by a client. Workflow Attributes
   * are not valid
   * 
   * @param aName
   * @return
   ...
   * 
   * If the object if from type Date or Calendar it will be formated unsing the Java
   * SimpleDateFormat.
   * 
   * If the object is String, Integer or Double the method tries to format the value into a number
   * 
   *
   * 
   * @param o
  ...
  private String formatObjectValue(Object o, String format, String locale) {
    ...
    if (dateValue != null) {
      if (format != null && !"".equals(format)) {
        ...
        singleValue =
            DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT).format(dateValue);
      }
  ...
  /**
   * This method converts a single item value into a specified type. If the converter is not
   * adaptable a default value will be set.
   *
   * 
   * @param o
   * @return
  ...
  /**
   * This method returns all embedded child items of a entity. The childItem are identified by a
   * fieldname containing a '~'. The left part is the container item (List of Map), the right part
   * is the attribute name in the child itemcollection.
   * 
   * @param entity
   * @param keySet
   * @return
   */
  @SuppressWarnings({"unchecked", "rawtypes"})
  private List<ItemCollection> getEmbeddedChildItems(ItemCollection entity,
        ...
        for (Object mapOderItem : mapChildItems) {
          if (mapOderItem instanceof Map) {
            ...
            for (String childFieldName : childFieldNameList) {
              clone.replaceItemValue(field + "~" + childFieldName,
                  child.getItemValue(childFieldName));
            }
            result.add(clone);
          }
        }
