...
/**
 * The ModelManager is independent form the IX JEE Entity EJBs and uses the standard IntemCollection
 * Object as a data transfer object to communicate with clients.
 * 
 * 
 * Since Version 1.7.0
 * 
 * The Implementation handles multiple model versions. Different Versions of an Model Entity can be
 * saved and updated. The Getter methods can be furthermore Controlled by providing a valid Model
 * Version. If no model version is set this Implementation automatically defaults to the highest
 * available ModelVersion
 * 
...
public class ModelService implements ModelManager {
  ...
  /**
   * This method initializes the modelManager and loads existing Models from the database. The
   * method can not be annotated with @PostConstruct because in case a servlet with @RunAs
   * annotation will not propagate the principal in a PostConstruct. For that reason the method is
   * called indirectly.
   * 
  ...
  /**
   * This Method adds a model into the internal model store. The model will not be saved in the
   * database! Use saveModel to store the model permanently.
   */
  @Override
  public void addModel(Model model) throws ModelException {
    ...
    if (definition == null) {
      throw new ModelException(ModelException.INVALID_MODEL,
          "Invalid Model: Model Definition not provided! ");
    }
    String modelVersion = definition.getModelVersion();
  ...
  /**
   * This method removes a specific ModelVersion form the internal model store. If modelVersion is
   * null the method will remove all models. The model will not be removed from the database. Use
   * deleteModel to delete the model from the database.
   * 
   * @throws AccessDeniedException
   */
  ...
  /**
   * Returns a Model by version. In case no matching model version exits, the method throws a
   * ModelException.
   **/
  @Override
  ...
  /**
   * Returns a Model matching a given workitem. In case not matching model version exits, the method
   * returns the highest Model Version matching the corresponding workflow group.
   * 
  ...
  public Model getModelByWorkitem(ItemCollection workitem) throws ModelException {
    ...
    try {
      ...
      if (modelVersion != null && !modelVersion.isEmpty()) {
        ...
        if (!versions.isEmpty()) {
          ...
          logger
              .info("...... match version '" + newVersion + "' -> by regex '" + modelVersion + "'");
      ...
      if (model == null && !workflowGroup.isEmpty()) {
        ...
        if (!versions.isEmpty()) {
          String newVersion = versions.get(0);
          logger.warning("Deprecated model version: '" + modelVersion + "' -> migrating to '"
              + newVersion + "',  $workflowgroup: '" + workflowGroup + "', $uniqueid: "
    ...
    if (model == null) {
      throw new ModelException(ModelException.UNDEFINED_MODEL_VERSION,
          "No matching $modelversion found for '" + modelVersion + "', $workflowgroup: '"
   ...
   * The result is sorted in reverse order, so the highest version number is the first in the result
   * list.
   * 
   * @param group
   * @return
  ...
  /**
   * This method saves a BPMNModel into the database and adds the model into the internal model
   * store.
   * <p>
   * If a model with the same model version exists in the database the old version will be deleted
   * form the database first.
   * 
   * @param model
  ...
  /**
   * This method saves a BPMNModel into the database and adds the model into the internal model
   * store. The model is attached as an embedded file with the given filename.
   * <p>
   * If a model with the same model version exists in the database the old version will be deleted
   * form the database first.
   * <p>
  ...
  /**
   * This method deletes an existing Model from the database and removes the model form the internal
   * ModelStore.
   * <p>
   * A model entity is identified by the type='model' and its name (model version). After the model
   * entity was deleted form the database, the method will also remove the model from the
   * ModelManager
   * 
   * @param model
   */
  public void deleteModel(String version) {
    if (version != null && !version.isEmpty()) {
      ...
      throw new InvalidAccessException(InvalidAccessException.INVALID_ID,
          "deleteModel - invalid model version!");
    }
  }

  /**
   * This method loads an existing Model Entities from the database. A model entity is identified by
   * its name (model version).
   * 
   * @param model
   */
