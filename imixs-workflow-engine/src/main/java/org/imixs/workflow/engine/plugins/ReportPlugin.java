...
/**
 * This plug-in executes a Imixs Report definition and stores the result either into the current
 * workitem ($file) or into the file system. The corresponding BPMN event provide the following
 * properties:
 * <p>
 * <ul>
 ...
 * CHANGES V 2.0
 * <p>
 * In the current version 2.0, only the processed document will be used as the xml input source for
 * the XSL transformation. A search query will currently not be evaluated.
 * 
 * 
 * @author Ralph Soika
 * @version 2.0
...
public class ReportPlugin extends AbstractPlugin {
   ...
   * <p>
   * The XML Source used by this method is the XML representation of the current document. The Query
  ...
  public ItemCollection run(ItemCollection adocumentContext, ItemCollection adocumentActivity)
    ...
    try {
      ...
      if (reportTarget.isEmpty() || "0".equals(reportTarget)) {
        FileData fileData =
            new FileData(reportFilePath, outputStream.toByteArray(), sContentType, null);
