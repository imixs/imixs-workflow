 ...
 * A Scheduler Implementation must implement the Interface
 * "org.imixs.workflow.engine.scheduler.Scheduler".
 * <p>
 * A scheduler definition is stored in a document with the type "scheduler". The document can
 * provide concrete information to process the timer event.
 * <p>
 * The TimerService can be started using the method start(). The Methods findTimerDescription and
 * findAllTimerDescriptions are used to lookup enabled and running service instances.
 * <p>
 * Each Method expects or generates a TimerDescription Object. This object is an instance of a
 * ItemCollection. To create a new timer the ItemCollection should contain the following attributes:
 * <p>
 * <ul>
 * <li>type - fixed to value 'scheduler'</li>
 * <li>_scheduler_definition - the chron/calendar definition for the Java EE timer service.</li>
 * <li>_scheduler_enabled - boolean indicates if the scheduler is enabled/disabled</li>
 * <li>_scheduler_class - class name of the scheduler implementation</li>
 * <li>_scheduelr_log - optional log information
 * </ul>
 * <p>
 * the following additional attributes are generated by the finder methods and can be used by an
 * application to verfiy the status of a running instance:
 * <ul>
 * <li>nextTimeout - Next Timeout - pint of time when the service will be scheduled</li>
...
public class SchedulerService {
  ...
  /**
   * Loads the scheduler configuration entity by name. The method returns null if no scheduler
   * configuration exits.
   * 
   * @return
   */
  public ItemCollection loadConfiguration(String name) {
    try {
      ...
      String sQuery = "(type:\"" + DOCUMENT_TYPE + "\" AND (name:\"" + name + "\" OR txtname:\""
          + name + "\" ) )";
  ...
  /**
   * This method saves the scheduler configuration. The method ensures that the following properties
   * are set to default.
   * <ul>
   * <li>type</li>
   * <li>name</li>
  ...
  public ItemCollection saveConfiguration(ItemCollection configItemCollection) {
    ...
    if (name == null || name.isEmpty()) {
      throw new InvalidAccessException(SchedulerService.class.getName(),
          SchedulerException.INVALID_WORKITEM,
   ...
   * <p>
   * The method throws an exception if the configuration entity contains invalid attributes or
   * values.
   * <p>
   * After the timer was started the configuration is updated with the latest statusmessage. The
   * item _schedueler_enabled will be set to 'true'.
   * <p>
   ...
   */
  public ItemCollection start(ItemCollection configuration)
    ...
    if (timer != null) {
      try {
        ...
        logger
            .warning("...failed to stop existing timer for '" + configuration.getUniqueID() + "'!");
        throw new InvalidAccessException(SchedulerService.class.getName(),
            SchedulerException.INVALID_WORKITEM, " failed to cancle existing timer!");
      }
    }

    logger.info("...Scheduler Service " + configuration.getUniqueID() + " will be started...");
    String schedulerDescription =
        configuration.getItemValueString(Scheduler.ITEM_SCHEDULER_DEFINITION);
  ...
  /**
   * Cancels a running timer instance. After cancel a timer the corresponding timerDescripton
   * (ItemCollection) is no longer valid.
   * <p>
  ...
  /**
   * This method will start all schedulers which are not yet started. The method is called for
   * example by the SchedulerStartupServlet.
   * 
   */
    ...
    try {
      ...
      for (ItemCollection schedulerConfig : col) {
        ...
        if (schedulerConfig != null
            && schedulerConfig.getItemValueBoolean(Scheduler.ITEM_SCHEDULER_ENABLED)) {
          try {
            ...
            if (findTimer(schedulerConfig.getUniqueID()) == null) {
              ...
              logger.info(
                  "...Scheduler Service " + schedulerConfig.getUniqueID() + " already running. ");
            }
          } catch (Exception e) {
            logger.severe("...start of Scheduler Service " + schedulerConfig.getUniqueID()
                + " failed! - " + e.getMessage());
            e.printStackTrace();
          }
        } else {
          logger
              .info("...Scheduler Service " + schedulerConfig.getUniqueID() + " is not enabled. ");
        }
      }
    } catch (QueryException e1) {
      e1.printStackTrace();
  ...
  /**
   * Updates the timer details of a running timer service. The method updates the properties
   * netxtTimeout and timeRemaining and store them into the timer configuration.
   * 
  ...
  /**
   * Creates a new log entry stored in the item _scheduler_log. The log can be writen optional to
   * the scheduler configuration and a workitem.
   * 
   * @param message
   * @param configuration
   */
  ...
  protected Scheduler findSchedulerByName(String schedulerClassName) {
    ...
    for (Scheduler scheduler : this.schedulerHandlers) {
      if (scheduler.getClass().getName().equals(schedulerClassName)) {
        if (debug) {
          logger
              .finest("......CDI Scheduler class '" + schedulerClassName + "' successful injected");
        }
        return scheduler;
      }
    }
   ...
   * The method calls the abstract method 'process' which need to be implemented by a subclass.
   * 
   * @param timer
   * @throws Exception
  ...
  protected void onTimeout(javax.ejb.Timer timer) {
    ...
    if (configuration == null) {
      logger.severe(
          "...failed to load scheduler configuration for current timer. Timer will be stopped...");
      timer.cancel();
      return;
    }
    ...
    try {
      ...
      if (scheduler != null) {
        logger
            .info("...run scheduler '" + id + "' scheduler class='" + schedulerClassName + "'....");
        Calendar cal = Calendar.getInstance();
  ...
  /**
   * Create a calendar-based timer based on a input schedule expression. The expression will be
  ...
  protected Timer createTimerOnCalendar(ItemCollection configItemCollection) throws ParseException {
    ...
    List<String> calendarConfiguation =
        configItemCollection.getItemValue(Scheduler.ITEM_SCHEDULER_DEFINITION);
