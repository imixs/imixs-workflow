...
/**
 * The JobHandlerRenameUser updates the name fields of workitems. A name can be replaced or added.
 * The following job attributes are expected:
 * 
 * <ul>
 * <li>namFrom - source userID</li>
 * <li>namTo - target userID</li>
 * <li>keyReplace - if true the source UserID will be replaced with the target UserID, otherwise the
 * target userID will be added</li>
 * </ul>
 * 
...
public class JobHandlerRenameUser implements JobHandler {
  ...
  public ItemCollection run(ItemCollection adminp) throws AdminPException {
    ...
    sQuery += " AND ($writeaccess:\"" + fromUserID + "\" OR $readaccess:\"" + fromUserID
        + "\" OR owner:\"" + fromUserID + "\" OR namowner:\"" + fromUserID + "\")";
    ...
    for (ItemCollection entity : col) {
      ...
      boolean result = ctx.getBusinessObject(JobHandlerRenameUser.class)
    ...
    logger.info("Job " + AdminPService.JOB_RENAME_USER + " (" + adminp.getUniqueID() + ") - "
        + colSize + " documents processed, " + iUpdates + " updates in " + time
        + " sec.  (in total: " + iProcessed + " processed, " + iUpdates + " updates)");
  ...
  public boolean updateWorkitemUserIds(ItemCollection entity, String from, String to,
    ...
    /*
     * if (updateList(entity.getItemValue("$Creator"), from, to, false)) bUpdate = true;
     */
