 ...
 * An EventLog defines a unique event created during the processing life-cycle of a workitem or the
 ...
 * </ul>
 * <p>
 * The 'data' attribute of an eventLog is optional and can hold any kind of event specific data
 * (e.g. a Mail Message).
 * <p>
 * EventLog entities can be created and accessed by the EventLogService. Typically a new EventLog
 * entity is created within the same transaction of the main processing or update life cycle. With
 * this mechanism a client can be sure that eventLogEntries returned by the EventLogService are
 * created during a committed Transaction.
 * <p>
 * Note: for the same document reference ($uniqueid) there can exist different eventlog entries.
 * Eventlog entries are unique over there internal ID.
 * 
