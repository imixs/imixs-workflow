 ...
 * <p>
 * An event that occurs during an update or a processing function within a transaction becomes a
 * fact when the transaction completes successfully. The EventLogService can be used to create this
 * kind of "Change Data Capture" events. An example is the LuceneUpdateService, which should update
 * the index of a document only if the document was successfully written to the database.
 * <p>
 * The service is bound to the current PersistenceContext and stores a defined type of document
 * entity directly in the database to represent an event. These types of events can be queried by
...
public class EventLogService {
   ...
   * The method returns an empty list if no event log entries exist of the given refId,
   * 
   * @param maxCount - maximum count of events to be returned
   * @param ref      - a reference ID for an assigned Document or Workitem instance
   * @param topic    - list of topics
   * 
   * @return - list of eventLogEntries
   */
  ...
  /**
   * Deletes an existing eventLog. The method catches javax.persistence.OptimisticLockException as
   * this may occur during parallel requests.
   * 
   * @param eventLog
   */
   ...
   * javax.persistence.OptimisticLockException as this may occur during parallel requests.
   * 
   * @param eventLog
   */
