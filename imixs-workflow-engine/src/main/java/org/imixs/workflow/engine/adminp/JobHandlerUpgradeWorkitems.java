...
public class JobHandlerUpgradeWorkitems implements JobHandler {
  ...
  /**
   * This method runs the RebuildLuceneIndexJob. The AdminP job description contains the start
   * position (numIndex) and the number of documents to read (numBlockSize).
   * <p>
   * The method updates the index for all affected documents which can be filtered by 'type' and
   * '$created'.
   * <p>
   ...
   * updated.
   * <p>
   * If the number of documents returned from the DocumentService is less the the BlockSize, the
   * method returns true to indicate that the Timer should be canceled.
   * <p>
  ...
  public ItemCollection run(ItemCollection adminp) throws AdminPException {
    ...
    List<ItemCollection> workitemList =
        documentService.getDocumentsByQuery(query, iIndex, iBlockSize);
    ...
    logger.info("Job " + AdminPService.JOB_UPGRADE + " (" + adminp.getUniqueID() + ") - " + colSize
        + " documents processed, " + iCount + " updates in " + time + " sec.  (in total: "
        + iProcessed + " processed, " + iUpdates + " updates)");
