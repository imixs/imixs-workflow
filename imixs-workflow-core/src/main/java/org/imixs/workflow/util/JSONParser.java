...
/**
 * The JSONParser is an utility class to parse JSON structures. The parser provides methods to
 * transfer a Imixs JSON structure into a Imixs ItemCollection as well as helper methods to extract
 * single values from a JSON structure.
 * <p>
 * The method parseWorkitem translates a JSON structure containing a Imixs Document into a
 * ItemCollection.
 * 
 * @author rsoika
 */
public class JSONParser {
  ...
  /**
   * This method extracts a single key from a JSON structure. It does not matter where the key is
   * defined within the JSON structure. The method simply returns the first match.
   * <p>
   * It is also possible to get a JSON object or an JSON array embedded in the given JSON structure.
   * This object can be parsed again with this method.
   * 
   * @param key
   * @param json
  ...
  /**
   * This method parses an Imixs JSON input stream and returns a Imixs ItemCollection.
   * 
   * Example: <code>
   *  {
  	"item":[
   ...
   */
  @Deprecated
  public final static ItemCollection parseWorkitem(final InputStream requestBodyStream,
      final String _encoding) throws ParseException, UnsupportedEncodingException {
  ...
  /**
   * This helper method extracts the type and value of a token and stores the value into the
   * workitem
   * 
   * e.g.
   * 
  ...
  private static void storeValue(String name, String token, ItemCollection workitem)
      throws ParseException {
    boolean debug = logger.isLoggable(Level.FINE);
  ...
  /**
   * This method finds the next position of a char. The method scips excapte characters like '\"' or
   * '\['
   * 
   * @param token
   * @param c
