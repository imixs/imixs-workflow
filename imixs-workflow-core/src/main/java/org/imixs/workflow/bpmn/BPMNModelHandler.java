...
/**
 * The Imixs BPMNDefaultHandler is used to extract the Imixs Task and Event Elements of a Imixs BPMN
 * model.
 * 
 * A BPMN file can either be a simple diagram with one process or a collaboration diagram with a
 * bpmn2:collaboration definition. For collaboration diagrams the currentWorkflowGroup is read from
 * the bpmn2:collaboration element. For a simple BPMN diagram type the currentWorkflowGroup is read
 * from the bpmn2:process element.
 * 
 * #issue 113: The parser connects pairs of catch and throw link events with a virtual SequenceFlow
 * to support the same behavior as if the link events were connected directly.
 * 
 * @author rsoika
 *
 */
public class BPMNModelHandler extends DefaultHandler {
  ...
  public void startElement(String uri, String localName, String qName, Attributes attributes)
    ...
    if (qName.equalsIgnoreCase("bpmn2:intermediateThrowEvent")
        && attributes.getValue("imixs:activityid") == null) {
      bThrowEvent = true;
    ...
    // parallel gateway
    if (qName.equalsIgnoreCase("bpmn2:exclusiveGateway")
        || qName.equalsIgnoreCase("bpmn2:inclusiveGateway")
  ...
  public void endElement(String uri, String localName, String qName) throws SAXException {
    ...
    if (bImixsEvent && (qName.equalsIgnoreCase("bpmn2:intermediateCatchEvent")
      ...
      if (currentSignalRefID != null && !currentSignalRefID.isEmpty()) {
        ...
        if (signalName != null && !signalName.isEmpty()) {
          ...
          logger.warning("Event " + currentEntity.getItemValueInteger("id") + " Signal Ref "
              + signalName + " is not defined!");
        }
      }
    ...
    if (bLinkThrowEvent && !bLinkCatchEvent
        && (qName.equalsIgnoreCase("bpmn2:linkEventDefinition"))) {
    ...
    // end of bpmn2:intermediateCatchEvent -
    if (bLinkCatchEvent && !bLinkThrowEvent
        && (qName.equalsIgnoreCase("bpmn2:linkEventDefinition"))) {
    ...
    if (bSequenceFlow && bconditionExpression
        && qName.equalsIgnoreCase("bpmn2:conditionExpression")) {
      String svalue = characterStream.toString();
  ...
  /**
   * This method builds the model from the information parsed by the handler. First all task
   * elements were adds as unique process entities into the model. In the second step the method
   * adds the Activity elements to the assigned Task. We look also for activities with no incoming
   * SequenceFlow.
   * 
   * The builder verifies the ProcessIDs for each task element to guaranty that the numProcessID is
   * unique
   * 
   * The build connects pairs of Catch and Throw LinkEvents with a virtual SequenceFlow to support
   * the same behavior as if those elements where connected directly in the model.
   * 
   * The method tests the model for bpmn2:message elements and replace links in Activity elements
   * attribute 'rtfMailBody'
   * 
  ...
  /**
   * This method returns the documentation of connected objects (e.g. annotations or dataObjects) to
   * a given Element
   * 
  ...
  /**
   * This method returns the documentations of connected dataObjects to a given Element
   * 
   * @param elementID - BPMN element linked with an annotation
  ...
  /**
   * This method returns all SourceTask Elements connected to a given eventID. The method takes care
   * about loop events and follow up events. Later ones are handled by the method addImixsEvent().
   * For that reason, the result of this method can be also an empty list.
   * 
   * An event can be a shared event so it is possible that more than one source tasks are found
   * 
   * @param eventID
   * @throws ModelException
   */
  private List<ItemCollection> findSourceTasks(String eventID) throws ModelException {
          ...
          if (sourceEvent != null) {
            ...
            if (startEvents.contains(aFlow.source)) {
              ...
              for (SequenceFlow outgoingFlow : outFlows) {
                targetTaskList =
                    new ElementResolver().findAllImixsTargetTaskIDs(outgoingFlow, targetTaskList);
              }
    ...
    logger.finest("......Imixs BPMN Event '" + eventID + "' is directly assigend to "
        + result.size() + " task elements");
    return result;
  }

  /**
   * This method computes the target for an event and adds the event to a source task. The method
   * call recursive if the target is a followUp Event.
   * 
   * If a event has no target the method throws an exception
   * 
   * If a event has more than one targets (task or event elements) then the event is handled as a
   * loop event.
   * 
   * If a event is already assigned to the sourceTask, the method returns without adding the event.
   * 
   * @param sourceTask
   * @param event
   * @throws ModelException
  ...
  private void addImixsEvent(String eventID, ItemCollection sourceTask) throws ModelException {
    ...
    if (event == null) {
      ...
      throw new ModelException(ModelException.INVALID_MODEL,
          "Imixs BPMN Event '" + eventID + "' unknown!");
    }
    // clone event
    ...
    try {
      if (model.getEvent(sourceTask.getItemValueInteger("numProcessID"),
        ...
        logger.finest(
            "......Imixs BPMN Event '" + eventName + "' is already assigned tosource task!");
        return;
      }
    ...
    if (event.getItemValueInteger("numprocessID") == event
  ...
  /**
   * This helper method verifies if the activity of the event is still unique for the task element.
   * If not the method computes a new one and updates the event
   * 
   * @param event
   * @param task
   * @return
  ...
  private ItemCollection verifyActiviytIdForEvent(ItemCollection event) {
    ...
    for (ItemCollection aactivity : assignedActivities) {
      ...
      if (aid == activityid) {
        ...
        logger.warning(
            "ActivityID " + name + " ID=" + activityid + " is not unique for task " + processid);
        activityid = -1;
      }
    }
  ...
  /**
   * This method returns an optional condition for a given sequenceFlow object. The method iterates
   * the conditionCache to lookup the condition
   * 
   * @param flow
  ...
  /**
   * This method parses an event for the text fragment <bpmn2:message>...</bpmn2:message> and
   * replaces the tag with the corresponding message if available
   * 
   * @param itemcol
   */
  ...
  /**
   * This is a helper method to adapt the old property names into the new. The method also works the
   * other way around so that new imixs-workflow can handle old bpmn files too.
   * 
  ...
  private void adaptDeprecatedTaskProperties(ItemCollection taskEntity) {
    ...
    adaptDeprecatedItem(taskEntity, BPMNModel.TASK_ITEM_ACL_OWNER_LIST_MAPPING,
        "keyownershipfields");
    adaptDeprecatedItem(taskEntity, BPMNModel.TASK_ITEM_ACL_READACCESS_LIST, "namaddreadaccess");
    adaptDeprecatedItem(taskEntity, BPMNModel.TASK_ITEM_ACL_READACCESS_LIST_MAPPING,
        "keyaddreadfields");
    adaptDeprecatedItem(taskEntity, BPMNModel.TASK_ITEM_ACL_WRITEACCESS_LIST, "namaddwriteaccess");
    adaptDeprecatedItem(taskEntity, BPMNModel.TASK_ITEM_ACL_WRITEACCESS_LIST_MAPPING,
        "keyaddwritefields");
  ...
  private void adaptDeprecatedEventProperties(ItemCollection eventEntity) {
    ...
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_WORKFLOW_PUBLIC_ACTORS,
        "keyrestrictedvisibility");

    // acl
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_ACL_OWNER_LIST, "namownershipnames");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_ACL_OWNER_LIST_MAPPING,
        "keyownershipfields");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_ACL_READACCESS_LIST, "namaddreadaccess");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_ACL_READACCESS_LIST_MAPPING,
        "keyaddreadfields");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_ACL_WRITEACCESS_LIST,
        "namaddwriteaccess");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_ACL_WRITEACCESS_LIST_MAPPING,
        "keyaddwritefields");
    ...
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_MAIL_TO_LIST_MAPPING,
        "keymailreceiverfields");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_MAIL_CC_LIST, "nammailreceivercc");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_MAIL_CC_LIST_MAPPING,
        "keymailreceiverfieldscc");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_MAIL_BCC_LIST, "nammailreceiverbcc");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_MAIL_BCC_LIST_MAPPING,
    ...
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_TIMER_DELAY_BASE,
        "keyscheduledbaseobject");
    adaptDeprecatedItem(eventEntity, BPMNModel.EVENT_ITEM_TIMER_DELAY_BASE_PROPERTY,
        "keytimecomparefield");
   ...
   * @param oldItemName
   */
  private void adaptDeprecatedItem(ItemCollection taskEntity, String newItemName,
      String oldItemName) {
  ...
  /**
   * This helper class provides methods to resolve the connected Imixs elements to a flow element.
   * The constructor is used to initialize a loopDetection cache
   * 
   * @author rsoika
   *
   */
  class ElementResolver {
    ...
    /**
     * This method searches a Imixs Task Element connected to the given SequenceFlow element. If the
     * Sequence Flow is not connected to a Imixs Task element the method returns null.
     * 
     * 
     * @return the Imixs Task element or null if no Task Element was found.
     * @return
     */
    public List<ItemCollection> findAllImixsSourceTasks(SequenceFlow flow,
        List<ItemCollection> sourceList) {
    ...
    /**
     * This method searches a Imixs Event Element connected to the given SequenceFlow element. If
     * the Sequence Flow is not connected to a Imixs Event element the method returns null.
     * 
     * 
    ...
    /**
     * This method searches a BPMN2:Start Event Element connected to the given SequenceFlow element.
     * If the Sequence Flow is not connected to a BPMN2:Start Event element the method returns null.
     * 
    ...
    /**
     * This method searches a BPMN2:End Event Element connected to the given SequenceFlow element.
     * If the Sequence Flow is not connected to a BPMN2:Event Event element the method returns null.
     * 
    ...
    /**
     * This method searches a Imixs Task Element targeted from the given SequenceFlow element. If
     * the Sequence Flow is not connected to a Imixs Task element the method returns null.
     * 
    ...
    /**
     * This method searches a Conditional Gateway targeted from the given SequenceFlow element. If
     * no conditional gateway was found the method returns null.
     * <p>
    ...
    /**
     * This method searches the id for a Imixs follow-Up activity. This is the case if the target is
     * another Imixs Event element. The method returns the id of the followup event
     * 
     * @return the ID of the Imixs Event element or null if no Event Element was found.
     * @return
     */
    ...
    /**
     * This method searches for all target events or task for a outgoing sequence flow. The method
     * returns a List of possible target elemetns.
     * 
     * 
     * @return the ID of the Imixs Event element or null if no Event Element was found.
     * @return
    ...
    /**
     * This method searches for all target tasks for a outgoing sequence flow. The method returns a
     * List of possible imixs task elements.
     * 
     * 
     * @return the ID of the Imixs Event element or null if no Event Element was found.
     * @return
