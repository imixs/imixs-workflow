...
public class EntityTableWriter implements MessageBodyWriter<EntityTable> {
		...
		try {
			...
			for (XMLItemCollection xmlworkItem : entityCollection.getEntity()) {
				...
				for (String attr : entityCollection.getAttributeList()) {
					...
					List vValues = itemCol.getItemValue(attr);
					/*
					 * String sValue = vValues.toString(); sValue =
					 * sValue.substring(1, sValue.length() - 1);
					 */
					bw.write("<td>"
							+ XMLItemCollectionWriter
									.convertValuesToString(vValues) + "</td>");
				}
				bw.write("</tr>");
	...
	public long getSize(EntityTable arg0, Class<?> arg1, Type arg2,
