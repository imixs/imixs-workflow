...
public class WorkflowRestService {
  ...
  public StreamingOutput getHelpHTML() {
    ...
    return new StreamingOutput() {
      public void write(OutputStream out) throws IOException, WebApplicationException {
        ...
        out.write(
            "table {padding:0px;width: 100%;margin-left: -2px;margin-right: -2px;}".getBytes());
        out.write(
            "body,td,select,input,li {font-family: Verdana, Helvetica, Arial, sans-serif;font-size: 13px;}"
                .getBytes());
        out.write(
            "table th {color: white;background-color: #bbb;text-align: left;font-weight: bold;}"
                .getBytes());
   ...
   * <p>
   * The file name will be encoded. With a URLDecode the filename is decoded in different formats
   * and searched in the file list. This is not a nice solution.
   * 
   * @param uniqueid
   * @return
   */
      ...
      if (workItem != null) {
        ...
        if (fileData != null) {
          ...
          Response.ResponseBuilder builder =
              Response.ok(fileData.getContent(), fileData.getContentType());
        ...
        logger.warning("WorklfowRestService unable to open file: '" + file + "' in workitem '"
            + uniqueid + "' - error: Workitem not found!");
        // workitem not found
        return Response.status(Response.Status.NOT_FOUND).build();
      }

    } catch (Exception e) {
      logger.severe("WorklfowRestService unable to open file: '" + file + "' in workitem '"
          + uniqueid + "' - error: " + e.getMessage());
      e.printStackTrace();
    }

    logger.severe(
        "WorklfowRestService unable to open file: '" + file + "' in workitem '" + uniqueid + "'");
    return Response.status(Response.Status.NOT_FOUND).build();
  ...
  public Response getEvents(@PathParam("uniqueid") String uniqueid,
  ...
  public Response getTaskListByOwner(@PathParam("owner") String owner,
    ...
    try {
      ...
      result =
          workflowService.getWorkListByOwner(owner, type, pageSize, pageIndex, sortBy, sortReverse);
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public Response getTaskListByAuthor(@PathParam("user") String user,
      @QueryParam("type") String type, @DefaultValue("0") @QueryParam("pageIndex") int pageIndex,
      @DefaultValue("10") @QueryParam("pageSize") int pageSize,
      @DefaultValue("") @QueryParam("sortBy") String sortBy,
      @DefaultValue("false") @QueryParam("sortReverse") Boolean sortReverse,
    ...
    try {
      ...
      result =
          workflowService.getWorkListByAuthor(user, type, pageSize, pageIndex, sortBy, sortReverse);
  ...
  public Response getTaskListByCreator(@PathParam("creator") String creator,
    ...
    try {
      ...
      result = workflowService.getWorkListByCreator(creator, type, pageSize, pageIndex, sortBy,
          sortReverse);
  ...
  public Response getTaskListByProcessID(@PathParam("processid") int processid,
      @QueryParam("type") String type, @DefaultValue("0") @QueryParam("pageIndex") int pageIndex,
      @DefaultValue("10") @QueryParam("pageSize") int pageSize,
      @DefaultValue("") @QueryParam("sortBy") String sortBy,
      @DefaultValue("false") @QueryParam("sortReverse") Boolean sortReverse,
    ...
    List<ItemCollection> result = null;
    try {
      result = workflowService.getWorkListByProcessID(processid, type, pageSize, pageIndex, sortBy,
          sortReverse);
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public Response getTaskListByGroup(@PathParam("processgroup") String processgroup,
      @QueryParam("type") String type, @DefaultValue("0") @QueryParam("pageIndex") int pageIndex,
      @DefaultValue("10") @QueryParam("pageSize") int pageSize,
      @DefaultValue("") @QueryParam("sortBy") String sortBy,
      @DefaultValue("false") @QueryParam("sortReverse") Boolean sortReverse,
    ...
    try {
      ...
      result = workflowService.getWorkListByGroup(processgroup, type, pageSize, pageIndex, sortBy,
          sortReverse);
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public Response getTaskListByRef(@PathParam("uniqueid") String uniqueid,
      @QueryParam("type") String type, @DefaultValue("0") @QueryParam("pageIndex") int pageIndex,
      @DefaultValue("10") @QueryParam("pageSize") int pageSize,
      @DefaultValue("") @QueryParam("sortBy") String sortBy,
      @DefaultValue("false") @QueryParam("sortReverse") Boolean sortReverse,
    ...
    List<ItemCollection> result = null;
    try {
      result = workflowService.getWorkListByRef(uniqueid, type, pageSize, pageIndex, sortBy,
          sortReverse);
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  /**
   * This method expects a form post and processes the WorkItem by the WorkflowService EJB. After
   * the workItem was processed the method redirect the request to the provided action URI. The
   * action URI can also be computed by the Imixs Workflow ResutlPlugin
   * 
  ...
  @Path("/workitem/{uniqueid : ([0-9a-f]{8}-.*|[0-9a-f]{11}-.*)}")
  @Consumes({MediaType.APPLICATION_FORM_URLENCODED})
  public Response postFormWorkitemByUnqiueID(@PathParam("uniqueid") String uid,
      InputStream requestBodyStream, @QueryParam("items") String items) {
  ...
  /**
   * This method post a ItemCollection object to be processed by the WorkflowManager. The method
   * test for the properties $taskidid and $eventid
   * 
   * NOTE!! - this method did not update an existing instance of a workItem. The behavior is
   * different to the method putWorkitem(). It need to be discussed if the behavior is wrong or not.
   * 
  ...
  public Response postWorkitemByUniqueID(@PathParam("uniqueid") String uniqueid,
      XMLDocument xmlworkitem, @QueryParam("items") String items) {
    logger.fine(
        "postWorkitemByUniqueID @POST /workitem/" + uniqueid + "  method:postWorkitemXML....");
    ItemCollection workitem;
  ...
  @Path("/workitem/{uniqueid : ([0-9a-f]{8}-.*|[0-9a-f]{11}-.*)}")
  @Consumes({MediaType.APPLICATION_XML, MediaType.TEXT_XML, MediaType.APPLICATION_JSON})
  public Response putWorkitemByUniqueID(@PathParam("uniqueid") String uniqueid,
      XMLDocument xmlworkitem, @QueryParam("items") String items) {
   ...
   * <p>
   * The Method returns a workitem with the new data. If a processException Occurs the method
   * returns an object with the error code
   * <p>
  ...
  /**
   * This method expects a form post. The method parses the input stream to extract the provides
   * field/value pairs. NOTE: The method did not(!) assume that the put/post request contains a
   * complete workItem. For this reason the method loads the existing instance of the corresponding
   * workItem (identified by the $uniqueid) and adds the values provided by the put/post request
   * into the existing instance.
   * 
   * The following kind of lines which can be included in the InputStream will be skipped
   * 
   * <code>
   * 	------------------------------1a26f3661ff7
          ...
          try {
            ...
            if (fieldValue.indexOf('=') == fieldValue.length()) {
              ...
              if (vMultiValueFieldNames.indexOf(fieldName) > -1) {
                ...
                v.add(fieldValue);
                logger.fine("[WorkflowRestService] multivalue for '" + fieldName + "' = '"
                    + fieldValue + "'");
  ...
  /**
   * This helper method processes a workitem. The response code of the response object is set to 200
   * if case the processing was successful. In case of an Exception a error message is generated and
   * the status NOT_ACCEPTABLE is returned.
   * <p>
  ...
  private Response processWorkitem(ItemCollection workitem, String uid, String items) {
    ...
    if (uid != null && !uid.isEmpty() && !workitem.getUniqueID().isEmpty()
        && !uid.equals(workitem.getUniqueID())) {
    ...
    try {
      if (workitem.hasItem("$error_code")) {
        ...
        return Response.ok(XMLDataCollectionAdapter.getDataCollection(workitem,
            DocumentRestService.getItemList(items))).build();
      }
    } catch (Exception e) {
      e.printStackTrace();
      return Response.status(Response.Status.NOT_ACCEPTABLE).build();
