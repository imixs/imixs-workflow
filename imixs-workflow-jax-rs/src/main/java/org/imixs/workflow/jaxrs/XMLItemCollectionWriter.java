...
public class XMLItemCollectionWriter implements MessageBodyWriter<XMLDocument> {
  ...
  public void writeTo(XMLDocument xmlItemCollection, Class<?> type, Type genericType,
  ...
  public long getSize(XMLDocument arg0, Class<?> arg1, Type arg2, Annotation[] arg3,
      MediaType arg4) {
    return -1;
  }

  /**
   * This Method prints a single XMLItemCollection in html format. The items are sorted by name
   * 
   * @param out
   * @param workItem
   * @throws IOException
  ...
  public static void printXMLItemCollectionHTML(BufferedWriter bw, XMLDocument xmlworkItem)
      throws IOException {
    ...
    bw.write("<tr class=\"a\">");
    bw.write("<th colspan=\"2\"><b>UniqueID: " + workItem.getItemValueString("$uniqueid")
        + "</b></th></tr>");
    bw.write("<tr class=\"a\">");
   ...
   * 
   * Multivalues will be separated with '~' characters. Date Objects will be converted into a short
   * String representation taking the server locale
   * 
   * 
   * @param values
   * @return
  ...
  /**
   * This method prints the generic HTML Header for HTML output, including a default CSS definition
   * for table layout. The meta tag Content-Type is generated in case a contentType and encoding is
   * provided.
   * 
   * @param bw
   * @throws IOException
   */
  public static void printHead(BufferedWriter bw, String contentType, String encoding)
      throws IOException {
    bw.write("<head>");

    if (contentType != null && encoding != null) {
      bw.write("<meta http-equiv=\"Content-Type\" content=\"" + contentType + "; charset="
          + encoding + "\"/>");
    }
    bw.write("<style>");
