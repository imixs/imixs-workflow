...
public class ReportRestService {
   ...
   * 
   * Since 2.1.2: the ReportService also supports FOP Transformation. If the ContentType is
   * 'application/pdf' the method will call fopTransofrmation instat of xslTransformation. The FOP
   * API need to be provided by the main application.
   * 
    ...
    try {
      ...
      if ("".equals(sXSL)) {
        Response.ResponseBuilder builder =
            Response.ok(XMLDataCollectionAdapter.getDataCollection(col), "text/html");
      ...
      /*
       * outputStream.toByteArray() did not work here because the encoding will not be considered.
       * For that reason we use the toString(encoding) method here.
       * 
       * 8.9.2012:
       * 
       * after some tests we see that only toByteArray will work on things like fop processing. So
       * for that reason we switched back to the toByteArray method again. But we still need to
  ...
  public Response getPdfReport(@PathParam("name") String reportName,
      ...
      @QueryParam("sortBy") String sortBy, @QueryParam("sortReverse") boolean sortReverse,
      @DefaultValue("") @QueryParam("encoding") String encoding, @Context UriInfo uriInfo) {
    return this.getExcecuteReport(reportName, pageSize, pageIndex, sortBy, sortReverse, encoding,
        uriInfo);
  }
  ...
  public DocumentTable getHTMLResult(@PathParam("name") String reportName,
    ...
    try {
      ...
      DocumentTable documentTable =
          new DocumentTable(documentCollection.getDocument(), items, labels, encoding);
  ...
  public Response getCustomResult(@PathParam("name") String reportName,
    ...
    DocumentTable documentTable = getHTMLResult(reportName, pageSize, pageIndex, sortBy,
        sortReverse, encoding, uriInfo, servlerResponse);
   ...
   * 
   * If a attribute list is defined in the report only the corresponding properties will be returend
   * in the xml stream.
   * 
   * If the query param 'items' is provided the attribute list in the report will be ignored.
   * 
   * @param name  reportname of the report to be executed
   * @param start
   * @param count
  ...
  public XMLDataCollection getJSONResult(@PathParam("name") String name,
    ...
    XMLDataCollection result = getXMLResult(name, pageSize, pageIndex, sortBy, sortReverse,
        encoding, uriInfo, servlerResponse);
   ...
   * @param outputWriter
   */
  public void fopTranformation(String xmlSource, String xslSource, String aEncoding,
  ...
  /**
   * This method parses the query Params of a Request URL and adds params to a given JPQL Query. In
   * addition the method replace dynamic date values in the JPQLStatement
   * 
   * 
   * @param uriInfo
   * @return
