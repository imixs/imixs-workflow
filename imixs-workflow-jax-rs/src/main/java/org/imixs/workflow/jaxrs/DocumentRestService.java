...
public class DocumentRestService {
  ...
  public StreamingOutput getRoot() {
    ...
    return new StreamingOutput() {
      public void write(OutputStream out) throws IOException, WebApplicationException {
        ...
        out.write(
            "<a href=\"/{uniqueid}\" type=\"application/xml\" rel=\"{uniqueid}\"/>".getBytes());
  ...
  /**
   * The method saves a document provided in xml format. The caller need to be assigned to the
   * access role 'org.imixs.ACCESSLEVEL.MANAGERACCESS'
   * 
   * Note: the method merges the content of the given document into an existing one because the
   * DocumentService method save() did not merge an entity. But the rest service typically consumes
   * only a subset of attributes. So this is the reason why we merge the entity here. In different
   * to the behavior of the DocumentService the WorkflowService method process() did this merge
   * automatically.
   * 
  ...
  public Response postDocument(XMLDocument xmlworkitem, @QueryParam("items") String items) {
    ...
    try {
      ...
      ItemCollection currentInstance =
    ...
    try {
      if (workitem.hasItem("$error_code")) {
        ...
        return Response
            .ok(XMLDataCollectionAdapter.getDataCollection(workitem), MediaType.APPLICATION_XML)
            .status(Response.Status.NOT_ACCEPTABLE).build();
      } else {
        return Response.ok(XMLDataCollectionAdapter.getDataCollection(workitem,
            DocumentRestService.getItemList(items)), MediaType.APPLICATION_XML).build();
      }
  ...
  /**
   * This method creates a backup of the result set form a JQPL query. The entity list will be
   * stored into the file system. The backup can be restored by calling the restore method
   * 
   * 
   * @param query
   * @param start
  ...
  public Response convertResult(ItemCollection workitem, String items, String format) {
    ...
    if ("json".equals(format)) {
      return Response
          // Set the status and Put your entity here.
          .ok(XMLDataCollectionAdapter.getDataCollection(workitem,
