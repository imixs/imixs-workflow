...
public class DocumentRestServiceV40 {
	...
	private static Logger logger = Logger.getLogger(DocumentRestServiceV40.class.getName());
	...
	public XMLItemCollection getDocument(@PathParam("uniqueid") String uniqueid, @QueryParam("items") String items) {
		...
		try {
			...
			return XMLItemCollectionAdapter.putItemCollection(document, DocumentRestServiceV40.getItemList(items));
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
